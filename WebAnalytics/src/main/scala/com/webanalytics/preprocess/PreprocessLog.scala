...
import org.apache.spark.sql.functions.{ udf, regexp_extract, translate, broadcast }
import org.apache.hadoop.hdfs.server.namenode.INodeReference.WithCount
import com.webanalytics.preprocess.UserDefinedFunctions.{ getUrl, ipToIntUDF, hostToCountry, encryptUrl }
...
object PreprocessLog {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession
      .builder()
      .master("local")
      .appName("Web Log Preprocess").config("spark.sql.broadcastTimeout",-1)
      .getOrCreate()
    ...
    val nullLogRecordDf = logFileDf.filter("host is null or timestamp is null or request is null or http_response_code is null or bytes is null")
    ...
    //Filtering the rejects
    val rejectDf = logDfDateCleaned.filter(!$"request".rlike(urlRegex))
    rejectDf.select("encrypted_host", "timestamp", "request", "http_response_code", "bytes").write.mode(SaveMode.Overwrite).csv(args(2) + "/reject_records")

    //Deriving the requested resource from the url request
    val logFileReqDf = urlCleanDf.withColumn("request_url", getUrl($"request"))
    logFileReqDf.registerTempTable("log_with_request")

    // Filtering the requests with just the images
    // MSFC, JPG, JPEG, MPG, GIF, XBM
    val mediaFilteredDf = logFileReqDf.filter(!$"request_url".rlike("(.jpg|.gif|.jpeg|.mpg|.mfsc|.xbm)$"))
    ...
    val mediaDataDf = logFileReqDf.filter($"request_url".rlike("(.jpg|.gif|.jpeg|.mpg|.mfsc|.xbm)$"))
    ...
    ipFileDf.registerTempTable("ip_country_data")
    ...
    ipFileReqDf.registerTempTable("ip_country_mapping_data")
    ...
    ipComputedDf.registerTempTable("log_with_long_ip")
    ...
    logsWithAlphIpWithCode.registerTempTable("log_with_cntry_code")
    ...
    val completeHostCountry = ipCountryDf.unionAll(hostCountryDfnotNA).distinct()
    ...
    //val countryMappedLogEnc= countryMappedLog.withColumn("encryptedIP", encryptUrl($""))

    countryMappedLog.select("encrypted_host", "timestamp", "request", "http_response_code", "bytes", "request_url", "country").write.mode(SaveMode.Overwrite).csv(args(2) + "/ip_country_mapping")

    //Creating and updating accumulator values
    val rejectRecordsAccum = spark.sparkContext.accumulator(0, "NumRejectedRecords")
    val mediaRecordsAccum = spark.sparkContext.accumulator(0, "NumMediaRecords")
    val cleanRecordsAccum = spark.sparkContext.accumulator(0, "NumCleanRecords")

    rejectDf.foreach(x => rejectRecordsAccum += 1)
    mediaDataDf.foreach(x => mediaRecordsAccum += 1)
    mediaFilteredDf.foreach(x => cleanRecordsAccum += 1)
